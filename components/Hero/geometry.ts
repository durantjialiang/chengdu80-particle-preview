...
export function networkCities(lowPower: boolean) {
  return siteContent.cities.filter(city => !lowPower || city.showOnLowPower);
}

export function networkRoutes(lowPower: boolean) {
  ...
  return cities.filter(city => !city.isOrigin).map(city => {
    ...
    const points = Array.from({ length: 57 }, (_, i) => {
      ...
      return start.clone().multiplyScalar(Math.sin((1 - t) * angle))
        .addScaledVector(end, Math.sin(t * angle)).normalize()
        .multiplyScalar(RADIUS + 0.026 + Math.sin(Math.PI * t) * (0.12 + angle * 0.095));
    });
...
const landMasks: number[][][] = [
  [[-168,71],[-142,70],[-128,58],[-123,49],[-126,41],[-117,32],[-107,29],[-97,16],[-84,9],[-77,9],[-87,22],[-80,25],[-81,31],[-65,47],[-54,52],[-61,61],[-83,63],[-101,73],[-134,70]],
  [[-81,12],[-70,11],[-60,5],[-50,0],[-35,-6],[-41,-22],[-51,-30],[-64,-55],[-72,-48],[-75,-28],[-80,-5]],
  [[-17,36],[3,37],[12,33],[31,31],[34,23],[43,12],[51,11],[42,0],[35,-15],[24,-35],[17,-33],[10,-15],[7,3],[-8,5],[-17,15]],
  [[-10,36],[-9,44],[-1,48],[-5,58],[8,58],[15,70],[29,71],[33,61],[48,68],[68,70],[91,76],[115,73],[144,71],[169,65],[179,53],[161,57],[143,47],[135,35],[122,30],[121,21],[108,19],[103,10],[99,3],[95,18],[89,23],[80,8],[73,20],[67,24],[57,25],[48,30],[43,13],[35,29],[28,41],[22,36],[16,41],[11,44],[3,42]],
  [[113,-22],[122,-14],[135,-12],[141,-16],[146,-17],[153,-28],[146,-39],[132,-34],[116,-35]],
  [[-53,60],[-44,60],[-20,76],[-31,83],[-54,81],[-62,71]],
  [[130,31],[134,34],[141,41],[145,44],[143,35],[137,33]],
  [[95,5],[105,-5],[116,-8],[130,-9],[140,-5],[123,1],[109,4]],
  [[47,-13],[50,-16],[47,-25],[44,-21]],
];
