...
function BrandOrbits({ lowPower, reducedMotion, clock }: LayerProps) {
  ...
  useFrame(() => {
    if (material.current) material.current.uniforms.uReveal.value = reveal(clock.current.elapsed, INTRO.atmosphere + 0.2);
...
function Routes({ lowPower, reducedMotion, clock }: LayerProps) {
  const routes = useMemo(() => networkRoutes(lowPower), [lowPower]);
  const packets = useRef<THREE.Points>(null);
  const point = useMemo(() => new THREE.Vector3(), []);
  const uniforms = useMemo(() => ({ uElapsed: { value: 0 }, uMotion: { value: 0 } }), []);
  const material = useRef<THREE.ShaderMaterial>(null);
  ...
  useFrame(() => {
    ...
    if (packets.current && !reducedMotion) {
      routes.forEach((curve, i) => {
        const elapsed = Math.max(0, clock.current.elapsed - INTRO.routes - i * 0.15 - 0.6);
    ...
    <group>
      <lineSegments>
        ...
        <shaderMaterial ref={material} uniforms={uniforms}
          ...
          fragmentShader={`uniform float uElapsed; uniform float uMotion; varying float vProgress; varying float vRoute;
            void main() {
              float reveal = clamp((uElapsed - 2.5 - vRoute * 0.15) / 0.6, 0.0, 1.0);
              float alpha = (1.0 - smoothstep(reveal - 0.04, reveal, vProgress)) * step(0.01, reveal);
              float packet = fract(max(0.0, uMotion - 3.1 - vRoute * 0.15) * 0.065);
              float trail = exp(-pow((vProgress - packet) * 30.0, 2.0));
              gl_FragColor = vec4(0.42, 0.78, 0.91, alpha * (0.19 + trail * 0.3));
            }`}
