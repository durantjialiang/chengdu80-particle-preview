...
import { networkRoutes, networkCities } from './geometry';
...
function BrandOrbits({ lowPower, reducedMotion, clock, opening }: LayerProps) {
  ...
  const { paths, positions, packetPositions } = useMemo(() => {
    ...
    return {
      ...
      packetPositions: new Float32Array(lowPower ? 3 : 6),
    };
  }, [lowPower]);
  ...
  useFrame(() => {
    if (material.current)
      material.current.uniforms.uReveal.value = opening
        ? reveal(opening.current.frame.globeRevealProgress, 0.18, 0.5)
        : reveal(clock.current.elapsed, INTRO.atmosphere + 0.2);
  ...
  const {
    ...
    routes.forEach((curve, index) => {
      ...
      for (let i = 1; i < points.length; i++) {
        ...
        ecosystem.push(
          destinations[index].isEcosystem ? 1 : 0,
          destinations[index].isEcosystem ? 1 : 0,
        );
      }
    });
    return {
      ...
      ecosystem: new Float32Array(ecosystem),
      packetPositions: new Float32Array(routes.length * 3),
      packetIndices: Float32Array.from(routes.map((_, index) => index)),
    };
  }, [routes, destinations, lowPower]);
  /* oxlint-disable react/react-compiler -- Frame updates write preallocated GPU buffers/uniforms, not React state. */
  useFrame(() => {
    if (material.current) {
      ...
      material.current.uniforms.uActivation.value = opening
        ? 0.28 + 0.72 * opening.current.frame.activationProgress
        : 1;
    ...
    <group>
      <lineSegments>
        ...
        <shaderMaterial
          ref={material}
          uniforms={uniforms}
          vertexShader={`attribute float aProgress; attribute float aRoute; attribute float aEcosystem; varying float vProgress; varying float vRoute; varying float vEcosystem;
            void main() { vProgress = aProgress; vRoute = aRoute; vEcosystem = aEcosystem; gl_Position = projectionMatrix * modelViewMatrix * vec4(position, 1.0); }`}
          fragmentShader={`uniform float uElapsed; uniform float uMotion; uniform float uActivation; uniform float uOpening; uniform float uSelected; varying float vProgress; varying float vRoute; varying float vEcosystem;
            void main() {
      ...
      {!reducedMotion ? (
        <points ref={packets}>
          <bufferGeometry>
            ...
            />
            <bufferAttribute
              attach="attributes-aRoute"
              args={[packetIndices, 1]}
            />
          </bufferGeometry>
          <shaderMaterial
            uniforms={uniforms}
...
export default function DataStreams(props: LayerProps) {
  return (
    <group>
      <Routes {...props} />
      {!props.network ? <BrandOrbits {...props} /> : null}
    </group>
  );
}
