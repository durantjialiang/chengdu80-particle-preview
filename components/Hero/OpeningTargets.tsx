...
export default function OpeningTargets({
  opening,
  globe,
  host,
  lowPower,
  ...
  const { coordinates, nodeCount } = useMemo(() => {
    ...
    const nodeSlots = Math.max(16, cities.length);
    ...
    for (let i = 0; i < HANDOFF_TARGETS; i++) {
      if (i < nodeSlots) {
        ...
        const lane = i - nodeSlots;
        routes[lane % routes.length].getPointAt(
          (Math.floor(lane / routes.length) + 0.5) /
            Math.ceil((208 - nodeSlots) / routes.length),
          point,
        );
      } else {
        const p = (i - 208 + 0.5) / 112;
        const y = 1 - p * 2,
          angle = (i - 208) * 2.399963;
        const radius = RADIUS * (1.04 + (i % 7) * 0.012);
        point
          .set(
            Math.sqrt(1 - y * y) * Math.cos(angle),
            y,
  ...
  useFrame(({ camera }) => {
    ...
    if (
      !host.current ||
      !globe.current ||
      target.frame.state === 'GLOBE_ACTIVE'
    )
      return;
    ...
    for (let i = 0; i < HANDOFF_TARGETS; i++) {
      point
        .fromArray(coordinates, i * 3)
        .applyMatrix4(globe.current.matrixWorld)
        .project(camera);
      target.targets[i * 3] = rect.left + (point.x * 0.5 + 0.5) * rect.width;
      target.targets[i * 3 + 1] =
