...
import {
  historyReviewPlugin,
  historyPublicationGuard,
} from './history-review-plugin';
...
export default defineConfig(({ mode, command }) => ({
  ...
  plugins: [
    ...
    historyReviewPlugin(
      ...
      process.env.CHENGDU80_HISTORY_REVIEW_DIR,
    ),
    {
      ...
      configureServer(server) {
        server.middlewares.use((req, _res, next) => {
          ...
          if (path === '/')
            req.url = '/qa/particle80.html' + (query ? '?' + query : '');
          else if (normalized === '/global-network/')
            req.url = '/qa/global-network.html' + (query ? '?' + query : '');
          else if (routes.some((route) => route.path === normalized))
            req.url = '/qa/site.html' + (query ? '?' + query : '');
          next();
        });
      },
    },
