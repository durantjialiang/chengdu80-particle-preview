...
import {
  ...
  HomeBeforeNetwork,
  HomeAfterNetwork,
} from '@/components/site/EcosystemContent';
  ...
  return (
    <>
      <main id="main-content">
        <h1 className="sr-only">CHENGDU 80</h1>
        {import.meta.env.DEV && reviewQuery?.get('record') === '1' ? (
          <OpeningReviewCursor />
        ) : null}
        <Particle80Intro
          headerContent={<SiteNavigation embedded />}
          footerContent={<HeroEssentials />}
          introduction={
            <>
              <p className={introStyles.eyebrow}>
                {t(
                  b(
                    'AN 80-HOUR GLOBAL FINTECH HACKATHON',
                    '80小时全球金融科技创新挑战',
                  ),
                )}
              </p>
              <h2 id="event-introduction-title">
                {t(b('Build the Future of Finance.', '共创金融未来。'))}
              </h2>
              <p className={introStyles.officialName}>
                {t(
                  b(
              ...
              <div className={introStyles.actions}>
                <a href={href('/about/')}>
                  {t(b('Discover Chengdu 80', '了解成都八零'))} ↗
                </a>
                <a href="#organizers">
                  {t(b('Academia meets industry', '连接学术与产业'))} ↓
                </a>
              </div>
            </>
          }
        ...
        <GlobalUniversityNetwork
          compact
          forceReducedMotion={still}
          staticPreview={reviewQuery?.get('renderer') === 'svg'}
        />
        <HomeAfterNetwork />
        {import.meta.env.DEV && (
          <section
          ...
          >
            <div
              className="intro-study-controls-inner"
            ...
            >
              ...
              <label htmlFor="intro-low">
                <Switch id="intro-low" checked={low} onCheckedChange={setLow} />{' '}
                Low power
              </label>
              <output>
                {phase ?? 'space'} · Formation{' '}
                {Math.round(formationProgress * 100)}% ·{' '}
