...
import Particle80Intro, { type HandoffContext } from '../components/Particle80Intro';
...
const fieldDebug =
  ...
  debugQuery === 'telemetry'
    ? debugQuery
    : 'off';
const reviewQuery = import.meta.env.DEV ? new URLSearchParams(window.location.search) : null;
function GlobeDestination({ opening, state, reducedMotion, lowPower }: HandoffContext) {
  const { pageVisible } = useScenePreferences();
  const active = state === 'DISSOLVING_80' || state === 'HANDOFF_TO_GLOBE' || state === 'GLOBE_ACTIVE';
  return (
    <section className="intro-globe-destination">
      <div className="intro-globe-copy">
        ...
        <p className="intro-network-label">A global university innovation network.<br />Originating in Chengdu.</p>
...
function Study() {
  ...
  const [automatic, setAutomatic] = useState(reviewQuery?.get('autoTransition') !== '0');
  ...
  return (
    <main>
      {import.meta.env.DEV && reviewQuery?.get('record') === '1' ? <OpeningReviewCursor /> : null}
      <Particle80Intro
        brightnessPreset={brightnessPreset}
        debug={fieldDebug}
        key={replay}
        repeatVisit={replay || reviewQuery?.get('visit') === 'first' ? 'always' : 'short'}
      ...
      >
        ...
        <label htmlFor="intro-auto"><Switch id="intro-auto" checked={automatic} onCheckedChange={setAutomatic} /> Auto transition</label>
        <output>
          {phase ?? 'space'} · Formation {Math.round(formationProgress * 100)}%
          · {settled ? 'Settled' : 'Forming'} · Interaction{' '}
          {interactionEnabled ? 'on' : 'off'}
          {' · '}{openingFrame?.state ?? 'INTRO_IDLE'} · Globe {Math.round((openingFrame?.globeRevealProgress ?? 0) * 100)}%
        </output>
      </section>
    </main>
  );
