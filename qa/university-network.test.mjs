  ...
  try {
    ...
    await t.test(
      ...
      () => {
        ...
        assert.equal(getUniversity('unsw').logo, '/university-logos/unsw.webp');
    ...
    await t.test(
      ...
      async () => {
        ...
        const html = renderToString(
          React.createElement(Network, { staticPreview: true }),
        );
        const summary = html.match(
          /<div[^>]*aria-live="polite"[^>]*>([\s\S]*?)<\/div>/,
        )?.[1];
        ...
        assert.doesNotMatch(
          summary,
          /<svg|Historical competition record|THE CONVERGENCE POINT|SELECTED UNIVERSITY/,
        );
        assert.doesNotMatch(
          html,
          /Hover a card\. Select a node\.|Selected historical records, not a confirmed 2026 roster|Sources &amp; record notes/,
        );
        assert.match(html, /University cards/);
        assert.match(html, /data-university="uestc"/);
      },
    );
    await t.test(
