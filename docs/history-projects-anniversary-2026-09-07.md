...
用户要求按作品亮点与专刊扩写每个年度的技术方案。七个项目各补充三段中英技术路径，年度页与项目页共用 `ProjectTechnicalApproach`，保留原生折叠交互，不影响粒子、地球和页面路线。

- 2018—2021：重新阅读专刊作品页，围绕已描述的原型设计组织数据、算法、架构与界面解释。不将专刊描述视为独立技术审计，不增加性能、安全或收益保证。
- 2022—2024：`technicalInterpretation: true`。在展开内容顶部明确显示“方案解读：依据公开作品亮点展开，非原团队已披露的实现细节。” 新增流程采用“可”“一种设计思路”等表述；不指定未知模型、数据供应商或实现参数。
- Giraffe 的输入检查、权限分层和评估流程，Apollo 的新闻字段、解释呈现与时间顺序评估，Data Queens 的场景定义、服务路径与原型走查，均属于编辑性设计解读，不升级为历史团队事实。
- Apollo / Data Queens 的团队名与产品名边界不变；不新增作品图片、整份专刊或其他素材公开授权。
- 回归检查新增七个项目各三段内容的中英文渲染、年度/详情一致性，以及三项方案解读标识的强制断言。
